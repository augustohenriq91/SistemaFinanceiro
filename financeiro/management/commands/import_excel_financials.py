...
<<<<<<< HEAD
        ...
        if keep_existing:
            self.stdout.write('Modo seguro ativado: os dados existentes serao mantidos.')
        elif not options['noinput']:
...
<<<<<<< HEAD
                ...
                if sheet_name not in wb.sheetnames:
                    self.stderr.write(f'Aba ṇo encontrada: {sheet_name}')
                    return
                ws = wb[sheet_name]
                if not self._process_sheet(ws, sheet_name):
                    self.stderr.write(f'Aba {sheet_name} ṇo p̫de ser importada.')
                    return
...
<<<<<<< HEAD
        ...
        self.contas = {
            ...
            'NuBank': Conta.objects.get_or_create(nome='NuBank', defaults={'tipo': 'banco', 'saldo_inicial': 0})[0],
            'Inter': Conta.objects.get_or_create(nome='Inter', defaults={'tipo': 'banco', 'saldo_inicial': 0})[0],
            'Santander': Conta.objects.get_or_create(nome='Santander', defaults={'tipo': 'banco', 'saldo_inicial': 0})[0],
            'Carteira': Conta.objects.get_or_create(nome='Carteira', defaults={'tipo': 'carteira', 'saldo_inicial': 0})[0],
        }

        self.categorias = {
            'Receitas': Categoria.objects.get_or_create(nome='Receitas', tipo='receita')[0],
            'Despesas': Categoria.objects.get_or_create(nome='Despesas', tipo='despesa')[0],
